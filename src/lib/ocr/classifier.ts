 ...
 *   - `imageDetail: "low"` cuts the image to a 512×512 crop server-
 *     side (OpenAI does this internally), which is plenty for logo +
 *     theme recognition and reduces vision-token cost ~3×.
 *   - `maxTokens: 80` — the response is `{"source":"yape","confidence":1}`,
 *     which is ~30 tokens. 80 leaves margin without bloat.
 *   - `timeoutMs: 15_000` — classification is fast (~600ms typical).
